...
for index, row in data.iterrows():
    ...
    data = data.drop(data.columns[0], axis=1)
    data['calc_value'] = data['calc_value'].astype(float).mul(np.where(data['category']=='from', -1, 1))
    ...
    df_grouped = df.groupby(['contract_address', 'month'])['calc_value'].sum().groupby('contract_address').cumsum()
    ...
    df_pivot = df_grouped.pivot(index='month', columns='contract_address', values='calc_value')
    ...
    full_index = pd.period_range(start=df_pivot.index.min(), end=current_month, freq='M')

    # Reindex the DataFrame to include all months and fill missing values with NaN
    ...
    def promote_rows_as_headers(df):
        ...
        new_columns = pd.MultiIndex.from_arrays([df.columns, *new_header.values])  # Create a MultiIndex for the new headers
...
for sheet_name in sheet_names:
    ...
    monthly = monthly.melt(id_vars=['date'], value_vars=monthly.columns[1:], var_name="contract_address", value_name="price")
    ...
    cols[0] = "date"
    chain.columns = cols
    chain = chain.melt(id_vars=['date'], value_vars=chain.columns[1:], var_name="contract_address", value_name="amount")
    chain["date"] = pd.to_datetime(chain["date"]).dt.to_period("m")

    merged_dataframe = pd.merge(monthly, chain, how="inner", on=['date', 'contract_address'])
    merged_dataframe['value_usd'] = merged_dataframe['price'] * merged_dataframe['amount']

    merged_dataframe = merged_dataframe.pivot(index='date', columns='contract_address')['value_usd']
